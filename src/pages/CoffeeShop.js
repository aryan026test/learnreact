...
const CoffeeShop = (prop) => {
  const {value} = prop
  const {ItemsNumber,setItemsNumber, CartItems,  setCartItems, pageItems, setPageItems} = value
  ...
  const CoffeeList = CoffeeInfo.map((element, index)=>{
    return <Card view={toggleView} CartItems={CartItems} setCartItems={setCartItems} ItemsNumber={ItemsNumber} setItemsNumber={setItemsNumber} pageItems={pageItems} setPageItems={setPageItems} info={element} key={index} />
  })
